...
The C64 Block Tutor currently provides a visual block-based programming experience where users create programs by assembling blocks (SET BORDER, SET BG, PRINT, SHOW SPRITE, WAIT, LOOP BACK). Behind the scenes, a dynamic code generator at `$3000` translates these blocks into 6502 machine code at `$5000` and executes it with `JSR $5000`.

**The Problem:** Users see their blocks and the results, but never see the assembly code or understand how their blocks translate to machine-level instructions. This hides the core learning objective - understanding 6502 assembly.
...
This creates the complete learning experience: users start with blocks, then peek behind the curtain to see real assembly code executing instruction-by-instruction.
...
**Strategy:** During code generation, build a parallel metadata array that tracks each generated instruction.
...
- `STATE_ASM_STEPPING (5)` - single-stepping through code with live register display
...
**T-Key Handler:** Add to all existing states (PALETTE, PROGRAM, EDIT_PARAM) to enter `STATE_ASM_VIEW`
...
5. **Test:** Assemble, run F1, use VICE monitor to inspect `$6000` and verify metadata
...
6. **Test:** Press S in assembly view → step through code with live register updates
...
1. Add row 20 renderer: "BLOCK #N: <NAME> (<PARAM>)"
...
After each phase:
1. **Assemble:** `java -jar bin/KickAss.jar src/main.asm -o build/main.prg -symbolfile`
...
4. **Automated test:** `bash test.sh` (golden screenshot should show block view, not asm view)
...
- [x] T key toggles between block view and assembly view
- [x] Assembly view shows accurate disassembly with syntax highlighting
- [x] S key enables single-step mode with live register display
- [x] Sound effects play on each step
- [x] Each instruction shows which block created it
- [x] User can learn assembly by seeing blocks → code → execution

---

## Implementation Status

### ✅ Phase 1: Metadata Foundation - COMPLETED
- Added ZP variables ($12-$1B)
- Added metadata buffer (ASM_META_BUF = $6000)
- Added state constants (STATE_ASM_VIEW, STATE_ASM_STEPPING)
- Implemented emit_instruction_meta() subroutine
- Modified all 6 emitters to track metadata
